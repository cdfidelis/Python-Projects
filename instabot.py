...
class InstaBot:
    ...
    def mainloop(self):
        while self.prog_run and self.login_status:
            ...
            self.thm = datetime.datetime.now()
			
            if self.thm.hour==0:
                self.today=0
                time.sleep(1*60*60)
                print(" Photo_var randomizer's sleep: 1hr ")
				
            if self.today==0:
                self.pixpst=random.randint(9,10)
                self.pixpst2=random.randint(11,12)
                self.pixpst3=random.randint(14,15)
                self.pixpst4=random.randint(18,19)
                self.today=1
				
            if self.thm.hour==self.pixpst and self.ptimelk ==0:
				#Post a picture by after 9
                print(" Time for photo upload")
                os.system("php uploadphoto.php")
                self.ptimelk=1
                print(" Photo..............")
                print(" Photo..............")
			
            if self.thm.hour==self.pixpst2 and self.ptimelk2 ==0:
				#Post a picture by after 11
                print(" Time for photo upload")
                os.system("php uploadphoto.php")
                self.ptimelk2=1
                print(" Photo..............")
            ...
            if self.thm.hour==self.pixpst3 and self.ptimelk3 ==0:
				#Post a picture by after 14
                print(" Time for photo upload")
                os.system("php uploadphoto.php")
                self.ptimelk3=1
                print(" Photo..............")
            ...
            if self.thm.hour==self.pixpst4 and self.ptimelk4 ==0:
				#Post a picture by after 18
                print(" Time for photo upload")
                os.system("php uploadphoto.php")
                self.ptimelk4=1								
                print(" Photo..............")
            ...
            if self.thm.hour==13:
				#1hr delay
                print(" Time = 13 GMT, sleeping for 1hr")
                time.sleep(1*60*60)
                self.timechk=0
                self.rand_inbtw_delay=0
                self.rand_wait_delay=0				
 
				
            if self.thm.hour ==17:
				#1hr delay
                print(" Time = 17 GMT, sleeping for 1hr")                
                time.sleep(1*60*60)
                self.timechk=0
                self.rand_inbtw_delay=0
                self.rand_wait_delay=0

            if self.like_counter >= 998: 
				#sleep for 5hrs if like=998+
                print("Like limit reached, total likes for today = 998)")               
                time.sleep(5*60*60) 

				
            if(self.timechk==0 and (dns == 0 or dne < dns) and dne != 0):
                self.timevar=time.time()
                self.timechk=1
                self.rand_wait_delay=random.randint(2,3) + 0.5
                self.rand_inbtw_delay=random.randint(12,13)
            ...
            if (time.time()-self.timevar>=self.rand_wait_delay*60 and
                (dns == 0 or dne < dns) and dne != 0
                ): #4*60 random.randint(3,5)*60
                print(" Sleeping for", self.rand_inbtw_delay, "mins")
                print(" Rand_Wait_Delay:", self.rand_wait_delay)	
                print(" Rand_Inbtw_Delay:", self.rand_inbtw_delay)
                time.sleep(self.rand_inbtw_delay*60)	#11*60 random.randint(10,12)*60
                self.timechk=0
                self.rand_inbtw_delay=0
                self.rand_wait_delay=0				
